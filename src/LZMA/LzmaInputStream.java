		...
		while (uncompressed_size < outSize) {
			...
			if (RangeDecoder.BitDecode(probs, IsMatch
				...
				int ind_prob = Literal
						+ (LZMA_LIT_SIZE * ((((uncompressed_size + GlobalPos) & literalPosMask) << lc) + ((previousbyte & 0xFF) >> (8 - lc))));
				...
				if (PreviousIsMatch) {
					...
					previousbyte = RangeDecoder.LzmaLiteralDecodeMatch(probs,
							ind_prob, matchbyte);
					PreviousIsMatch = false;
				} else {
					previousbyte = RangeDecoder.LzmaLiteralDecode(probs,
							ind_prob);
				}
				...
				if (RangeDecoder.BitDecode(probs, IsRep + State) == 1) {
					if (RangeDecoder.BitDecode(probs, IsRepG0 + State) == 0) {
						if (RangeDecoder.BitDecode(probs, IsRep0Long
								+ (State << CRangeDecoder.kNumPosBitsMax)
								+ posState) == 0) {
					...
					RemainLen = RangeDecoder.LzmaLenDecode(probs, LenCoder,
							posState);
					int posSlot = RangeDecoder
							.BitTreeDecode(
									probs,
									PosSlot
											+ ((RemainLen < kNumLenToPosStates ? RemainLen
