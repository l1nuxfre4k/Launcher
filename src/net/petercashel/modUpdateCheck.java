...
public class modUpdateCheck {
	...
	public static void doUpdateCheck() {
		...
		if ((new File((binDir + "version.csv"))) != null) {
			...
			try {
				try {
					CSVReader reader = new CSVReader(new FileReader(binDir
							+ "version.csv"));
					String[] nextLine;
					while ((nextLine = reader.readNext()) != null) {
						// downloadFile(( "http://assets.minecraft.net/" +
						// nextLine[0] + "/minecraft.jar"
						// ),binDir,"base_minecraft.jar");
						modpackVersion = Integer.parseInt(nextLine[0]);
					}
				} catch (IOException e) {
					e.printStackTrace();
				}
				CSVReader reader = new CSVReader(new FileReader(tmpDir
						+ "version.csv"));
				String[] nextLine;
				while ((nextLine = reader.readNext()) != null) {
					// downloadFile(( "http://assets.minecraft.net/" +
