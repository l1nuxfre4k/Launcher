...
public class GameUpdater implements Runnable {
    ...
    protected String[] genericErrorMessage = { "An error occured while loading the applet.", "Please contact support to resolve this issue.",
	    "<placeholder for error message>" };
    protected boolean certificateRefused;
    protected String[] certificateRefusedMessage = { "Permissions for Applet Refused.", "Please accept the permissions dialog to allow",
	...
	switch (state) {
		...
		// I HAZ STATE!
	    case 11:
		return modUpdater.stateString;
	    case 12:
		return launcherUpdater.stateString;
	    case 13:
		return modUpdateCheck.stateString;
	    case 14:
	...
	try {
		...
		// Check for mod package updates
		state = 13;
		modUpdateCheck.doUpdateCheck();

		// Run the mod updater if the user pressed the button
		if (modUpdater.modUpdate) {
		    state = 11;
		    modUpdater.run();
		}
		...
		if (f.exists()) {
		    FileUtils.copyFile((new File(jarSwapdir + "built_minecraft.jar")), (new File(jarSwapdir + "minecraft.jar")));
		}
	    }
